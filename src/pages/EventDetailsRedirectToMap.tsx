...
const EventDetails = () => {
  ...
  const event = useMemo(() => events.find((ev) => ev.id === parseInt(eventId || '')), [events, eventId]);
  ...
  const changeRequestStatus = async (targetUserId: number, newStatus: string) => {
    await supabase
      .from('event_requests')
        ...
        <div className="mt-10">
          ...
          <div>
            ...
            <div className="space-y-2">
              {accepted.map((r) => (
                <div key={r.user.id} className="p-3 bg-green-50 border border-green-200 rounded-lg text-gray-700">
                  {r.user.username}
                </div>
              ))}
              {accepted.length === 0 && <p className="text-gray-400 text-sm">No accepted attendees.</p>}
            </div>
          </div>
          ...
          {isHost && (
            <>
              <div className="mt-6">
                ...
                <div className="space-y-2">
                  {pending.map((r) => (
                    <div key={r.user.id} className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex justify-between items-center">
                      <span className="text-gray-700">{r.user.username}</span>
                      <div className="flex gap-2">
                        <button
                          onClick={() => changeRequestStatus(r.user.id, 'accepted')}
                          className="px-3 py-1 bg-green-500 text-white text-sm rounded-lg"
                        >
                          Accept
                        </button>
                        <button
                          onClick={() => changeRequestStatus(r.user.id, 'rejected')}
              ...
              <div className="mt-6">
                ...
                <div className="space-y-2">
                  {rejected.map((r) => (
                    <div key={r.user.id} className="p-3 bg-red-50 border border-red-200 rounded-lg text-gray-700">
                      {r.user.username}
                    </div>
                  ))}
                  {rejected.length === 0 && <p className="text-gray-400 text-sm">No rejected attendees.</p>}
                </div>
              </div>
            </>
          )}
