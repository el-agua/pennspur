...
const MAPBOX_TOKEN = "pk.eyJ1IjoiZHl6aGFvNzM3OSIsImEiOiJjbTEybWdwY3QwYzd2MmlvamdyMTZ6Z2p0In0.bBlKaVU-2ZO7gWdv1UHdEg"; 
...
const CreateEvent = () => {
  ...
  useEffect(() => {
    setFormData(prev => ({ ...prev, emoji: EMOJI_OPTIONS[currentEmojiIndex] }));
  }, [currentEmojiIndex]);
  ...
  const handleOpenMapPicker = () => {
    ...
    if (mapRef.current) {
        const targetLng = parseFloat(formData.longitude || viewState.longitude.toString());
        const targetLat = parseFloat(formData.latitude || viewState.latitude.toString());
        // @ts-ignore
        ...
        setViewState(prev => ({ ...prev, longitude: targetLng, latitude: targetLat, zoom: 13 }));
    }
  };
  ...
  const handlePreviousEmoji = () => {
      setCurrentEmojiIndex((prevIndex) => (prevIndex - 1 + EMOJI_OPTIONS.length) % EMOJI_OPTIONS.length);
  };
        ...
        <div 
          className={`relative z-10 w-full min-h-screen pt-4 flex justify-center items-start 
                      transition-opacity duration-700 ease-in-out ${bottomPaddingClass} ${
                        openMap ? 'opacity-0 pointer-events-none' : 'opacity-100'
                      }`}
        >
            <div 
            ...
            >
                ...
                <form onSubmit={handleSubmit} id="create-event-form">
                    ...
                    <div className="mb-4">
                        <span className="text-gray-700 font-medium block mb-2">Event Icon</span>
                        <div className="flex items-center justify-center p-3 rounded-xl bg-white/90 border border-gray-300 relative">
                            ...
                            <span className="text-5xl transition-transform duration-300 transform scale-100 hover:scale-110 cursor-pointer"
                ...
                <div className="fixed bottom-0 left-0 right-0 p-4 pb-20 z-[95] flex justify-center">
                    <div className="w-full max-w-md bg-white/70 backdrop-blur-md rounded-xl p-4 shadow-xl border border-white/30">
                        ...
                        <div className="text-center text-xs text-gray-600 mt-2">
                            Current Center: {viewState.latitude.toFixed(4)}, {viewState.longitude.toFixed(4)}
                        </div>
                    </div>
                </div>
            </div>
