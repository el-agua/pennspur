...
const EventDetails = ({ event, userId }: EventDetailsProps) => {
  const eventStatus = useMemo(
    () => event.requests.find((request) => request.user.id === userId)?.status,
    [event, userId],
  );
  ...
  return (
    <div>
      <div className="flex justify-between">
        ...
        <button
          className="bg-blue-400 rounded-lg p-2 text-sm cursor-pointer hover:bg-blue-500 hover:scale-110 transition-all"
      ...
      <p className="mb-2">
        Started{" "}
        {Math.round(
          (new Date().getTime() - event.startTime.getTime()) / (1000 * 60),
        )}{" "}
        minutes ago
      </p>
